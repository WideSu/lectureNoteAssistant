...
func (app *VideoSrt) Run(video string) {
	...
	if app.OutputType.SUM {
		AliyunAudioResultMakeSubtitleFile(app, video, OUTPUT_SUMMARY, AudioResult, IntelligentBlockResult)
	}
...
func AliyunAudioResultMakeSubtitleFile(app *VideoSrt, video string, outputType int, AudioResult map[int64][]*aliyun.AliyunAudioRecognitionResult, IntelligentBlockResult map[int64][]*aliyun.AliyunAudioRecognitionResult) {
	...
	for channel, result := range thisAudioResult {
		...
		var txtTransalteOutputContent bytes.Buffer //翻译结果
		...
		for _, data := range result {
			...
			} else if outputType == OUTPUT_STRING || outputType == OUTPUT_SUMMARY {
				//普通文本处理
				txtOutputContent.WriteString(data.Text) //主字幕
				txtOutputContent.WriteString("\r\n")    //回车换行

				if app.TranslateCfg.TranslateSwitch { //如果选择双语字幕
					txtTransalteOutputContent.WriteString(data.TranslateText) //翻译后结果
		...
		intoSentences := 3
		bag := tldr.New()
		result, _ := bag.Summarize(data.Text, intoSentences)
		fmt.Println(result) */

		//写入文本文件（一次性写入以免耗时）
		if outputType == OUTPUT_STRING || outputType == OUTPUT_SUMMARY {
			txtOutputContent.WriteString("\r\n\r\n\r\n\r\n\r\n")
